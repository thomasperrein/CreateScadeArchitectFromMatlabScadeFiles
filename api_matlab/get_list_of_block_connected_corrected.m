function output = get_list_of_block_connected_corrected(filepath)
    ...
    for k=1:numel(obj)
        ...
        if strcmp(get_param(a,'BlockType'),'Inport')
            ...
            e{end+1,1}.port_associated = input_port_1{index,1};
            for j=1:length(obj2)
                if strcmp(obj2(j).Type, type) && isempty(obj2(j).DstBlock)
                        ...
                        for num=1:numel(temp_port_co)
                            if isempty(temp_port_co(num).DstBlock)
                                if strcmp(get_param(temp_port_co(num).SrcBlock,'BlockType'),'From')
                                    ...
                                    e{end,1}.name = {get_param(structure{1,1}.SrcBlock,'Name')};
                                else
                                    e{end,1}.name = {get_param(temp_port_co(num).SrcBlock,'Name')};
                                end
                            end
                        end
                    elseif strcmp(get_param(obj2(j).SrcBlock,'BlockType'),'From')
                        ...
                        e{end,1}.name = {get_param(obj2(j).SrcBlock,'Name')};
                    end
                end
            end
            index = index + 1;
    ...
    for k=1:numel(obj)
        ...
        if strcmp(get_param(a,'BlockType'),'Outport')
            ...
            e{end+1,1}.port_associated = output_port_1{index,1};
            for j=1:length(obj2)
                if strcmp(obj2(j).Type, type) && isempty(obj2(j).SrcBlock)
                        ...
                        for num=1:numel(temp_port_co)
                            if isempty(temp_port_co(num).SrcBlock)
                                if strcmp(get_param(temp_port_co(num).DstBlock,'BlockType'),'Goto')
                                    ...
                                    structure = get_param(name,'PortConnectivity');
                                    e{end,1}.name = {get_param(structure{1,1}.DstBlock,'Name')};
                                else
                                    e{end,1}.name = {get_param(temp_port_co(num).DstBlock,'Name')};
                                end
                            end
                        end
                    elseif strcmp(get_param(obj2(j).DstBlock,'BlockType'),'Goto')
                        ...
                        for el_name=1:numel(name)
                            ...
                            for u=1:length(structure{1,1}.DstBlock)
                                if strcmp(get_param(structure{1,1}.DstBlock(u),'BlockType'),'UnitDelay') | strcmp(get_param(structure{1,1}.DstBlock,'BlockType'),'Delay') | strcmp(get_param(structure{1,1}.DstBlock,'BlockType'),'Demux') | strcmp(get_param(structure{1,1}.DstBlock,'BlockType'),'Selector')
                                    tempo = get_param(structure{1,1}.DstBlock(u),'PortConnectivity');
                                    curr_name = '';
                                    for m=1:numel(tempo)
                                        if isempty(tempo(m).SrcBlock) & ~strcmp(get_param(tempo(m).DstBlock,'Name'),curr_name)
                                            one_cell{end+1,1} = get_param(tempo(m).DstBlock,'Name');
                                            curr_name = get_param(tempo(m).DstBlock,'Name');
                                        end
                                    end
                                else
                                    one_cell{end+1,1} = get_param(structure{1,1}.DstBlock(u),'Name');
                                end
                            end
                        end
                        e{end,1}.name = one_cell;
                    else
                        e{end,1}.name = {get_param(obj2(j).DstBlock,'Name')};
                    end
                end
            end
            index = index + 1;
